package com.testando.gateway.security.oauth2;

import com.testando.gateway.web.rest.errors.InvalidPasswordException;
...
public class OAuth2AuthenticationService {
     ...
     * @param autorization the Header Param
    ...
    public ResponseEntity<OAuth2AccessToken> authenticate(HttpServletRequest request, HttpServletResponse response,
                                                          Map<String, String> params, String autorization) {
        try {
            String username = params.get("username");
            String password = params.get("password");
            boolean rememberMe = Boolean.valueOf(params.get("rememberMe"));
            log.info("#############Remember me ? {}", rememberMe);
            OAuth2AccessToken accessToken = authorizationClient.sendPasswordGrant(username, password, autorization);
    ...
    /**
     * Try to refresh the access token using the refresh token provided as cookie.Note that browsers typically send multiple requests in parallel which means the access token
 will be expired on multiple threads.
     * We don't want to send multiple requests to UAA though,
